...
import { WandIcon, DownloadIcon } from './icons/Icons';
...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({ user, updateCredits, addToHistory }) => {
  ...
  const cost = user.isAdmin ? 0 : IMAGE_GENERATION_COST;
  ...
  const handleGenerate = async () => {
    ...
    if (user.credits < cost) {
      setError(`You need at least ${cost} credits to generate an image.`);
      return;
    }
    ...
    try {
      ...
      addToHistory({ prompt, imageUrl, timestamp: Date.now() });
    } catch (err) {
  ...
  const handleDownload = () => {
    if (generatedImage) {
      ...
      link.download = `${prompt.slice(0, 20).replace(/\s/g, '_') || 'generated_image'}.jpeg`;
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fadeIn">
        <div className="bg-light-surface dark:bg-dark-surface p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800/50">
          <h2 className="text-2xl sm:text-3xl font-bold mb-1 text-light-primary dark:text-dark-primary">Image Generation</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-6">Describe the image you want to create. Your imagination is the only limit.</p>
          ...
          {user.credits < cost && !isLoading && (
             <p className="text-red-500 text-center mt-4 text-sm">You need at least {cost} credits to generate an image.</p>
          )}
        ...
        {generatedImage && (
          <div className="mt-8 bg-light-surface dark:bg-dark-surface p-4 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800/50 animate-fadeIn">
            <div className="aspect-video rounded-xl overflow-hidden relative group">
                <img ref={imageRef} src={generatedImage} alt={prompt} className="w-full h-full object-cover" />
                 <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                   <button onClick={handleDownload} className="p-3 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/30 transition-colors" aria-label="Download Image">
                     <DownloadIcon />
                   </button>
                 </div>
            </div>
